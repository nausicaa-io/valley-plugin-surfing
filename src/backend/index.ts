...
export function createFilters(api: PluginBackendApi) {
  ...
  const readProfiles = async (): Promise<void> => {
    if (configured) return
    const rows = await api.data.dataset('profile_adblock').query({ limit: 1000 })
    for (const row of rows.rows) {
      const settings = normalizeProfileAdblock(row)
      if (typeof row.profileId === 'string') profiles.set(partitionFor(row.profileId), { partition: partitionFor(row.profileId), enabled: settings.adblockEnabled, rules: settings.adblockRules, frequencyDays: settings.adblockFrequency })
    }
    configured = true
  }
  const fetchList = async (url: string, frequencyDays: number) => {
    ...
    try {
      ...
      await api.data.files.writeTextGuarded(path, JSON.stringify(record), previous?.baseline ?? null)
  ...
  const engine = async (partition: string): Promise<FiltersEngine | null> => {
    await readProfiles()
    const configuration = profiles.get(partition) ?? { partition, enabled: true, rules: DEFAULT_ADBLOCK_RULES, frequencyDays: 7 }
    if (!configuration.enabled) return null
    const key = `${configuration.frequencyDays}\n${configuration.rules}`
    ...
    if (!pending) {
      ...
      pending = (urls.length ? FiltersEngine.fromLists(fetcher, urls, { enableCompression: true }) : FiltersEngine.fromPrebuiltAdsAndTracking(fetcher)).catch((error) => { engines.delete(key); throw error })
